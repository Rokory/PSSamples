...
"@
    ...
    switch ($PSItem.FullyQualifiedErrorId) {
        'SqlException,Connect-SqlServer' { Write-Warning 'Could not connect to Assets database' }
        Default {
            throw
        }
    }
    ...
    Write-Warning $PSItem.Exception.GetType()
    Write-Warning $PSItem.FullyQualifiedErrorId
    throw
}
finally {
    if ($null -ne $sqlConnection -and $sqlConnection.State -ne 'Closed') {
        $sqlConnection.Close()
    }
}
